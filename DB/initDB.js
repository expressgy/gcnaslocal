...
class initDB{
    ...
    //  初始化全局文件表
    createGlobalFileTable(){
        const createGlobalFileSQL = 'Create Table If Not Exists file_global(' +
        ...
        this.connection.query(createGlobalFileSQL,(err,data) => {
            if(err) throw {TB:'file_global',code:err.code,errno:err.errno,sqlMessage:err.sqlMessage}
            console.log(colorConsole.succeed+'[INIT TABLE] : file_global INIT SUCCEED'+colorConsole.end)
        })
    }
    //  初始化普通用户信息表
    createUserInfoTable(){
